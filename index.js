...
/**
 * --- CONFIGURATION ---
 * Ensure you set the FIREBASE_SERVICE_ACCOUNT environment variable on your hosting provider
 * with the contents of your NEW Firebase Service Account JSON file.
 */
const dbURL = "https://deepa-aaf4d-default-rtdb.firebaseio.com";
const saContent = process.env.FIREBASE_SERVICE_ACCOUNT;

if (!saContent) {
  console.error("************************************************************");
  console.error("FATAL ERROR: FIREBASE_SERVICE_ACCOUNT is missing!");
  console.error("Action: Go to your Hosting Dashboard -> Environment Variables");
    ...
    <html>
      <body style="font-family: sans-serif; background: #0f172a; color: white; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0;">
        <div style="background: #1e293b; padding: 2rem; border-radius: 1rem; text-align: center; border: 1px solid #334155;">
          <h1 style="margin: 0; color: #10b981;">FCM Service Active</h1>
          <p>Target Database: <code style="color: #3b82f6;">${dbURL}</code></p>
        </div>
      </body>
    </html>
  `);
